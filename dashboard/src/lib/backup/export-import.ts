...
export async function exportDatabase(exportedBy: string): Promise<BackupData> {
  ...
  const backupUserApiKeys: BackupUserApiKey[] = userApiKeys.map((ak) => ({
    ...
    fastEnabled: ak.fastEnabled,
    policyEnabled: ak.policyEnabled,
    allowedModels: ak.allowedModels,
    fallbackProvider: ak.fallbackProvider,
    fallbackModel: ak.fallbackModel,
  }));
  ...
  while (true) {
    ...
    for (const ur of chunk) {
      backupUsageRecords.push({
        ...
        executorType: ur.executorType,
        serviceTier: ur.serviceTier,
        responseServiceTier: ur.responseServiceTier,
        requestId: ur.requestId,
        accountingVersion: ur.accountingVersion,
        accountingQuality: ur.accountingQuality,
        inputTotalTokens: ur.inputTotalTokens,
        uncachedInputTokens: ur.uncachedInputTokens,
        cacheReadTokens: ur.cacheReadTokens,
        cacheWriteTokens: ur.cacheWriteTokens,
        outputTotalTokens: ur.outputTotalTokens,
        nonReasoningOutputTokens: ur.nonReasoningOutputTokens,
        unclassifiedTokens: ur.unclassifiedTokens,
        failed: ur.failed,
    ...
    if (backup.data.userApiKeys.length > 0) {
      await tx.userApiKey.createMany({
        data: backup.data.userApiKeys.map((ak) => ({
          ...
          fastEnabled: ak.fastEnabled ?? false,
          policyEnabled: ak.policyEnabled ?? false,
          allowedModels: ak.allowedModels ?? [],
          fallbackProvider: ak.fallbackProvider ?? null,
          fallbackModel: ak.fallbackModel ?? null,
        })),
      });
    }
    ...
    for (const chunk of usageChunks) {
      await tx.usageRecord.createMany({
        data: chunk.map((ur) => ({
          ...
          accountingVersion: ur.accountingVersion ?? 1,
          accountingQuality: ur.accountingQuality ?? "legacy",
          inputTotalTokens: ur.inputTotalTokens ?? 0,
          uncachedInputTokens: ur.uncachedInputTokens ?? 0,
          cacheReadTokens: ur.cacheReadTokens ?? 0,
          cacheWriteTokens: ur.cacheWriteTokens ?? 0,
          outputTotalTokens: ur.outputTotalTokens ?? 0,
          nonReasoningOutputTokens: ur.nonReasoningOutputTokens ?? 0,
          unclassifiedTokens: ur.unclassifiedTokens ?? 0,
          failed: ur.failed,
